...
export default function Projects({ externalFilter }: { externalFilter?: string }) {
  const [activeFilter, setActiveFilter] = useState(externalFilter ?? "All");
  const [current, setCurrent] = useState(0);
  const [direction, setDirection] = useState(1);
  const autoRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Sync when externalFilter changes
  useEffect(() => {
    if (externalFilter !== undefined) setActiveFilter(externalFilter);
  }, [externalFilter]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12">
      ...
      {/* ── Standalone header — hidden when used inside ProjectShowcase ── */}
